  ...
  setup() {
    ...
    const exportExcel = () => {
      ...
      const ws = XLSX.utils.json_to_sheet(
        lowStockProducts.value.map((p, i) => ({
          No: i + 1,
          Nama: p.name,
          Barcode: p.barcode,
          Stok: p.stock
      ...
      XLSX.writeFile(wb, `low_stock_${new Date().toISOString().slice(0,10)}.xlsx`);
    };
    ...
    const exportPDF = () => {
      ...
      const doc = new jsPDF();
      const columns = ['No', 'Nama', 'Barcode', 'Stok'];
      ...
      doc.save(`low_stock_${new Date().toISOString().slice(0,10)}.pdf`);
    };
