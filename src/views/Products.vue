        ...
        <div class="modal-content">
          <form @submit.prevent="saveProduct">
            ...
            <div class="modal-body">
              ...
              <table class="table table-sm">
                ...
                <tbody>
                  <tr v-for="(u, index) in form.units" :key="index">
                    <td><input v-model="u.unit_name" class="form-control form-control-sm" required /></td>
                    <td><input v-model.number="u.quantity" type="number" class="form-control form-control-sm" required /></td>
                    <td><input v-model.number="u.price_regular" type="number" class="form-control form-control-sm" required /></td>
                    <td><input v-model.number="u.price_member" type="number" class="form-control form-control-sm" required /></td>
                    <td>
                      <button type="button" class="btn btn-sm btn-danger" @click="removeUnitRow(index)">
                        <i class="bi bi-trash"></i>
                      </button>
                    </td>
                  </tr>
              ...
              <button type="button" class="btn btn-sm btn-outline-primary" @click="addUnitRow()">
                + Tambah Satuan
              </button>
            </div>
...
methods: {
  ...
  openModal(product = null) {
    if (product) {
      ...
      window.api.fetchUnits(product.id).then(units => { this.form.units = units })
    } else {
      this.form = {
        id: null,
        name: '',
        ...
        stock: 0,
        min_stock: 0,
        units: [ { unit_name: 'pcs', quantity: 1, price_regular: 0, price_member: 0 } ]
      }
    }
    this.$nextTick(() => { this.modal = new Modal(this.$refs.modalEl); this.modal.show() })
  },
  async saveProduct() {
    // Destructure form data to plain objects to avoid Vue Proxy cloning issues
    const productData = {
      id: this.form.id,
      name: this.form.name,
  ...
  confirmDelete(id) {
    ...
    }).then(async res => { if (res.isConfirmed) { await window.api.deleteProduct(id); Swal.fire('Dihapus', 'Produk telah dihapus', 'success'); this.loadProducts() } })
  },
  addUnitRow() { this.form.units.push({ unit_name: '', quantity: 1, price_regular: 0, price_member: 0 }) },
  removeUnitRow(index) { this.form.units.splice(index, 1) },
